...
class VAE:
    ...
    def time_experiment(self, train_loader, test_loader=None, EPOCHS=1):
        """
        Conclusion - GPU is a bit faster
        """
        start_times_dict = {}
        durations_dict = {}
        epoch_per_decay = int(EPOCHS / 10)
        ...
        for EPOCH in range(EPOCHS):
            ...
            test_running_log_p_x_given_z = 0
            test_running_log_p_z = 0
            start_times_dict["EPOCH"] = time.time()
            for i, (x,) in enumerate(train_loader):
                #start_times_dict["push_x_to_device"] = time.time()
                start_times_dict["BATCH"] = time.time()
                x = x.to(self.device)
                #durations_dict["push_x_to_device"] = time.time() - start_times_dict["push_x_to_device"]

                start_times_dict["forward_pass"] = time.time()
                reconstruction_logits, log_q_z_given_x, log_p_z = self.VAE_model(x)
                """
                start_times_dict["encoder_forward_pass"] = time.time()
                z, log_q_z_given_x, log_p_z = self.VAE_model.encoder(x)
                durations_dict["encoder_forward_pass"] = time.time() - start_times_dict["encoder_forward_pass"]
                start_times_dict["dencoder_forward_pass"] = time.time()
                reconstruction_logits = self.VAE_model.decoder(z)
                durations_dict["dencoder_forward_pass"] = time.time() - start_times_dict["dencoder_forward_pass"]
                """
                durations_dict["forward_pass"] = time.time() - start_times_dict["forward_pass"]

                #start_times_dict["loss_calc"] = time.time()
                loss, log_p_x_given_z_per_batch, log_q_z_given_x_per_batch, log_p_z_per_batch = \
                    self.loss_function(reconstruction_logits, log_q_z_given_x, log_p_z, x)
                #durations_dict["loss_calc"] = time.time() - start_times_dict["loss_calc"]
                #start_times_dict["zero_grad"] = time.time()
                self.optimizer.zero_grad()
                #durations_dict["zero_grad"] = time.time() - start_times_dict["zero_grad"]
                start_times_dict["loss_backward"] = time.time()
                loss.backward()
                durations_dict["loss_backward"] = time.time() - start_times_dict["loss_backward"]
                #start_times_dict["gradient_step"] = time.time()
                self.optimizer.step()
                #durations_dict["gradient_step"] = time.time() - start_times_dict["gradient_step"]

                # this takes long
                """
                start_times_dict["running_info_train"] = time.time()
                ...
                durations_dict["running_info_train"] = time.time() - start_times_dict["running_info_train"]
                """
                durations_dict["BATCH"] = time.time() - start_times_dict["BATCH"]
                print(durations_dict)
            durations_dict["EPOCH"] = time.time() - start_times_dict["EPOCH"]
            # More stuff
            """
                start_times_dict["encoder_forward_pass2"] = time.time()
                z, log_q_z_given_x, log_p_z = self.VAE_model.encoder(x)
                durations_dict["encoder_forward_pass2"] = time.time() - start_times_dict["encoder_forward_pass2"]
                start_times_dict["dencoder_forward_pass2"] = time.time()
                reconstruction_logits = self.VAE_model.decoder(z)
                durations_dict["dencoder_forward_pass2"] = time.time() - start_times_dict["dencoder_forward_pass2"]
                """
            """

            if EPOCH % epoch_per_decay == 0 and EPOCH > 5:
                print("learning rate decayed")
                self.optimizer.param_groups[0]["lr"] *= 0.5
...
if __name__ == "__main__":
    ...
    vae = VAE(latent_dim=32, n_IAF_steps=2, h_dim=20, IAF_node_width=450, encoder_fc_dim=450, decoder_fc_dim=450, use_GPU=False)
    #vae.train(EPOCHS = 1, train_loader=train_loader) #, test_loader=test_loader)
    durations = vae.time_experiment(train_loader)
    print(f"number of batches is {len(train_loader)}")
    print(durations)
