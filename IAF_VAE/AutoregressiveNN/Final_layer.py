...
if __name__ == "__main__":
    ...
    lay = torch.nn.utils.weight_norm(FinalLayerMask(latent_dim=3, layer_width=7))
    print(lay(test_tensor))

    # test middle layer
    layer = FinalLayer(latent_dim=4, layer_width=6)
    print(layer(test_tensor))

    # debug checks
    # first axis must have zeros as previously all neurons were connected to it
    assert torch.sum(layer.layer_to_layer.autoregressive_mask[:, 0]) == 0
    # last axis was connected to all latent elements besides itself, so must not be zeros
    assert torch.sum(layer.layer_to_layer.autoregressive_mask[:, -1]) != 0
