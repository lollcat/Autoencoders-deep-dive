...
class FirstLayer(nn.Module):
    def __init__(self, latent_dim, h_dim, layer_width):
        super(FirstLayer, self).__init__()
        self.latent_to_layer = torch.nn.utils.weight_norm(
            FirstLayerMask(latent_dim=latent_dim, layer_width=layer_width))
        self.h_to_layer = torch.nn.utils.weight_norm(
            nn.Linear(h_dim, layer_width))

    def forward(self, z, h):
        x = self.latent_to_layer(z) + self.h_to_layer(h)
        return F.elu(x)
...
if __name__ == "__main__":
    ...
    h_test_tensor = torch.tensor([[1.5, 4.7, 5]])
    first_layer = FirstLayer(latent_dim=z_test_tensor.shape[1], h_dim=h_test_tensor.shape[1], layer_width=6)
    print(first_layer(z_test_tensor, h_test_tensor))
