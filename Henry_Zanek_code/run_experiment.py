...
X_train = x_train
X_test = x_test
from Henry_Zanek_code.iaf_vae import IAF_VAE_Class
if __name__ == "__main__":
    fig_path= "Henry_Zanek_code/Figures"
        ...
        for i, images in enumerate(train_ds):
            ...
            if i % 10 == 0:
                print(
                    f'Epoch {epoch + 1}, '
                    f'ELBO is {ELBO}'
                    f'\n log_prob_x_given_z_decode_batch: {ELBO_component_history_train[epoch, 0]}'
                    f'\n log_probs_z_given_x_batch {ELBO_component_history_train[epoch, 1]}'
                    f'\n log_prob_z_prior {ELBO_component_history_train[epoch, 2] }')
