...
import numpy as np
from Standard_VAE.VAE import VAE
import torch
...
if __name__ == "__main__":
    ...
    name = current_time + f"latent_2"
    fig_path = f"Standard_VAE/Figures/{name}/"
    pathlib.Path(os.path.join(os.getcwd(), fig_path)).mkdir(parents=True, exist_ok=True)

    train_loader, test_loader = load_data(256)
    vae = VAE(latent_dim=2, encoder_fc_dim=120, decoder_fc_dim=120, use_GPU=True)
